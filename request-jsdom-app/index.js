...
const jsdom = require('jsdom'); // web-js in node.js
...
function requestTopTeamsByLeague(leagueId, amount, callback) {
  request(`https://www.skysports.com/${leagueId}-table`, (err, res, body) => { // body - Print the HTML for a some page.
    if (err) {
      console.log('error: ', err);
      return;
    }
  
    const dom = new jsdom.JSDOM(body); // стрингу надиляем DOM-функционалом или преобразуем в DOM idk
